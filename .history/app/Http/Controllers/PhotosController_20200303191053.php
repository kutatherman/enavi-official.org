...
class PhotosController extends Controller
{
    ...
    //    return redirect('/gallery/' . $request->input('gallery_id'));
        ...
        $photos = Photo::where(galleiry_id,$request->album_id)->get();
        //$album = Gallery::all();
        $gallery=Gallery::find($request->album_id);
        $description = $gallery->description;
        return view('gallery.show', compact('photos', 'description'));
        //return redirect("gallery/$request->album_id");
        //return redirect()->route('galleryshow', ['id' => $request->album_id,'description'=>$description]);
